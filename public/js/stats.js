...
var Stats = function(){
	...
	var latencySum = 0, latencySum_2 = 0, latencyQty = 1;
	...
	function activateStreamingStats(){
		setInterval(function(){
			...
			if (READY_TO_SEND){
				...
				stats.latencyMean = mean(latencySum, latencyQty);
				stats.latencyStdev = stdev(latencySum, latencySum_2, latencyQty);
	...
	function addPacketDetails(ts, arr, pt, rpt, buff){
		if (first_time){
			...
			latencySum += Math.abs(arr - ts);
			latencySum_2 += Math.pow(Math.abs(arr-ts),2);
			latencyQty += 1;
		}
		tsPrev = ts;
		arrPrev = arr;
